...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/AkiSister/wiki/Home) | [English Home](https://github.com/alkaid616/AkiSister/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `AkiSister`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `AkiSister.json` and update `id`, `name`, `author`, and `description`.
3. Edit `AkiSisterCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `AkiSister/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `AkiSister.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `AkiSister.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\AkiSister.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/AkiSister` directory.
...
```text
AkiSister/
├── AkiSisterCode/   # C# source
├── AkiSister/       # Godot resources, localization, and placeholder scenes
├── AkiSister.csproj
├── AkiSister.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://AkiSister/...` is the Godot/PCK resource path, mapping to the repository resource directory `AkiSister/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `AkiSisterCharacter` |
| Expected ID | `AKI_SISTER_CHARACTER_AKI_SISTER_CHARACTER` |
| Starter deck | 4 × `AkiSisterStrike`, 4 × `AkiSisterDefend`, 1 × `AkiSisterRelic` |
...
|---|---|---|
| `AkiSisterStrike` (attack) | character card pool | `AKI_SISTER_CARD_AKI_SISTER_STRIKE` |
| `AkiSisterDefend` (skill) | character card pool | `AKI_SISTER_CARD_AKI_SISTER_DEFEND` |
| `AkiSisterRelic` | `AkiSisterRelicPool` | `AKI_SISTER_RELIC_AKI_SISTER_RELIC` |

### Static placeholder assets

**Images** (`res://AkiSister/images/...`):

- `cards/AkiSisterStrike.png`, `cards/AkiSisterDefend.png`: sample card art.
- `relics/AkiSisterRelic.png`: sample relic icon.
- `characters/AkiSister_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://AkiSister/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `AkiSister_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `AkiSister_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `AkiSister_merchant.tscn` | Merchant | — |
| `AkiSister_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `AkiSister_character_select_bg.tscn` | Character select background | — |
...
`AkiSister.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
